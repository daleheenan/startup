...
export class ChapterOrchestratorService {
  ...
  queueBookGeneration(bookId: string): {
    ...
    for (const chapter of chapters) {
      const jobs = this.queueChapterWorkflow(chapter.id);
      jobsCreated += Object.keys(jobs).length;
   ...
   *
   * Pipeline (variable based on project configuration):
   ...
   * 7. sensitivity_review - Sensitivity Reader checks representation (optional)
   * 8. research_review - Researcher validates facts/history (optional)
   * 9. beta_reader_review - Beta Reader simulates reader engagement (optional)
   ...
   */
  queueChapterWorkflow(chapterId: string): Record<string, string> {
    // Get chapter and project to determine agent configuration
    ...
    const projectStmt = db.prepare<[string], { generation_mode: string; selected_agents: string | null }>(`
      SELECT p.generation_mode, p.selected_agents
      FROM projects p
      JOIN books b ON b.project_id = p.id
      WHERE b.id = ?
    `);
    const project = projectStmt.get(chapter.book_id);

    const generationMode = (project?.generation_mode || 'publication') as GenerationMode;
    const customAgents = project?.selected_agents ? JSON.parse(project.selected_agents) : undefined;

    // Get agents for this mode
    const enabledAgents = agentConfigurationService.getModeAgents(generationMode, customAgents);

    logger.info({
      chapterId,
      generationMode,
      agentCount: enabledAgents.length,
    }, '[ChapterOrchestrator] Queueing chapter workflow');

    // Create jobs only for enabled agents
    const jobIds: Record<string, string> = {};

    for (const agentType of enabledAgents) {
      const jobId = QueueWorker.createJob(agentType, chapterId);
      jobIds[agentType] = jobId;
      logger.info(`  - ${agentType}: ${jobId}`);
    }

    logger.info({
      chapterId,
      totalJobs: enabledAgents.length,
      mode: generationMode,
    }, '[ChapterOrchestrator] Chapter workflow queued');
   ...
   */
  regenerateChapter(chapterId: string): Record<string, string> {
